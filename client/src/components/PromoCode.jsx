...
const mapDispatchToProps = (dispatch) => {
  return {
    addPromo: item => dispatch(addPromo(item)),
  };
};
...
class ConnectedPromoCode extends React.Component {
  ...
  handleSubmit(event) {
    const valueLowerCased = this.state.value.toLowerCase();
    if (!this.props.isChecked) {
      if (valueLowerCased === 'donation') {
        alert('A promo was submitted: ');
    ...
    return (
      <div>
        ...
        :
          <div>
            <span 
              onClick={this.promoCodeClicked} 
              className="item-container" 
              style={{textDecoration:'underline', paddingBottom:'0px', border: '.5px solid rgb(187, 185, 185)', cursor:'pointer'}}>
            Hide promo code
            </span>
